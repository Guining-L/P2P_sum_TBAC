...
import threading
import sys
from peer_control import PeerTurnOFF


def socket_service(name, peer_addr_dict):
    try:
    ...
    while 1:
        try:
            ...
            t = threading.Thread(target=receive_and_send_data_gram, name=thr_start_times,args=(conn, addr, state_dict, lock, peer_addr_dict,name,s,))
            t.start()


def receive_and_send_data_gram(conn, addr, state_dict, lock, peer_addr_dict, peer_name,peer_socket):
    ...
    if type(recv_object) == QueryDatagram:
        ...
        lock.release()
        (state_dict[recv_object.request_id]).Query_Datagram_Source_addr_List.append(recv_object.send_peer_id)
        print(recv_object.send_peer_id)
        if first_time_flag:
    ...
    if type(recv_object) == EvaluationDatagram:
        ...
        if first_time_recv_eva_flag:
            ...
            try:
                s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                first_peer = (state_dict[recv_object.request_id].Query_Datagram_Source_addr_List)[0]
    ...
    try:
        ...
        print("当前会话ID", state_dict[recv_object.request_id].request_id)
        print("这个会话状态被查询的节点", state_dict[recv_object.request_id].queried_peer)
        print("查询报文来源列表", state_dict[recv_object.request_id].Query_Datagram_Source_addr_List)
        print("当前会话评估报文列表", state_dict[recv_object.request_id].Evaluation_Datagram_List)
